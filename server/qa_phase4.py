...
c = api("/api/benchmark/PROP_9e3b1d18")
r = c.get("readout", "")
...
check("peer-group helper present", "You're comparing against" in appjs)
check("search helper present", "Type to find any of the 778" in appjs)
check("Ask lumi conversational hint present", "Ask in plain English" in appjs)
check("filter helpers present", "Show only one kind of question" in pages and "Filter by membership tier" in pages)
cards = open("../web/js/card.js").read()
check("buttons say what they do (Download/Copy/Expand/Pin titles)",
      all(t in cards for t in ("Download PNG", "Copy PNG", "Expand chart", "Pin to My view")))

print("== 4.5 Sample-data honesty ==")
ov = api("/api/overview"); meth = api("/api/methodology")
check("overview flags synthetic pool", ov.get("synthetic_pool") is True)
check("methodology flags synthetic pool", meth.get("synthetic_pool") is True)
check("client renders the label on both", "Illustrative sample data" in pages and "synthetic seed data" in pages)
