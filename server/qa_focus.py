...
HIDDEN = ["Processes","Wellbeing","Growth","Capability","Inclusivity","Attract","Leadership","Purpose","Change"]
def leak(blob):
    t = json.dumps(blob)
    return [h for h in HIDDEN if '"%s"' % h in t]

st, me = api("/api/me")
check("scope = Reward only, count 180", me["scope"]["superpowers"] == ["Reward"] and me["scope"]["question_count"] == 180, me["scope"])
st, qi = api("/api/questions")
check("/api/questions serves exactly 180, all Reward", len(qi["questions"]) == 180 and not leak(qi))
...
check("/api/benchmarks/Reward serves 180 cards", len(rw["cards"]) == 180)
st, ov = api("/api/overview")
check("overview clean of hidden areas", not leak(ov), leak(ov))
check("overview headline reward-scoped (comparable < 200)", ov["headline"]["comparable_metrics"] <= 180,
      ov["headline"]["comparable_metrics"])
check("by_superpower keys == {Reward}", list(ov["headline"]["by_superpower"].keys()) == ["Reward"])
check("by_section has the 5 reward sections", sorted(ov["headline"]["by_section"].keys()) ==
      sorted(["Pay","Benefits","Incentives","Transparency","Progression"]), sorted(ov["headline"]["by_section"].keys()))
...
check("methodology scope = reward sections", meth["scope"]["focused"] and
      sorted(meth["scope"]["sections"]) == sorted(["Pay","Benefits","Incentives","Transparency","Progression"]))
