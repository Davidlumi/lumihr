# -*- coding: utf-8 -*-
"""migrate_r3sw4_leave_floor.py — PENSION_TYPE public-sector carve-out (TPR A).
...
Dry-run default; --write requires --confirmed-by-david. Emits r3sw4_seed_manifest.csv.
"""
...
FLOOR_BANDS = {"Retail", "Hospitality", "Logistics"}
STAMP = "2026-07-18 r3sw4 leave floor"
PROF = {}
...
def main():
    ...
    bad = []
    for q in ("PROP_fe1a29ec", "REW_FAI_128", "REW_PAY_001", "REW_INC_103", "REW26_BEN_PENSION_TYPE"):
        ...
        nm = maturity_assign(q, GEN[q], rows, PROF, lat)
        curmap = dict(rows)
        # partial-coverage aware: a surgical-band entry (r3sw3 contract) only covers
        # its declared bands — backcompat = agreement over the newmap's own domain.
        if any(curmap.get(o) != v for o, v in nm.items()):
            bad.append(q)
    ...
    pub = {o for o, _ in rows if canon_industry((PROF.get(o) or {}).get("Industry") or "") in FLOOR_BANDS}
    assert set(newmap) == pub, "surgical-band contract breached: newmap must cover ONLY the floor bands"
    ...
    # floor-band coherence: modal must be Statutory-20 in EVERY floor band
    from collections import defaultdict
    bandcnt = defaultdict(Counter)
    for o, v in newmap.items():
        bandcnt[canon_industry((PROF.get(o) or {}).get("Industry") or "")][v] += 1
    for b, cnt in bandcnt.items():
        assert cnt.most_common(1)[0][0] == "Statutory minimum only (20 days)", (b, dict(cnt))
    tgt = dict(after)
    if not a.write:
    ...
    with open(os.path.join(ROOT, "r3sw4_seed_manifest.csv"), "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["metric_id", "action", "before", "after"])
        w.writeheader()
        w.writerow({"metric_id": QID, "action": "annual-leave sector floor (R/H/L modal 20, surgical-band)",
    ...
    print(json.dumps({"applied": True, "public_band": dict(pubdist), "changed_orgs": len(changed),
                      ...
                      "non_target_book": "hash-identical", "manifest": "r3sw4_seed_manifest.csv"}, indent=2))
