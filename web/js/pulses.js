  ...
  return html`
    <div class="pulse-report-doc" style=${{ margin: "var(--s4) 0" }}>
      ...
      <div class="card">
        ...
        ${report.questions.map((q, i) => html`<${PulseQuestionBlock} key=${q.question_id} q=${q} pid=${pid} me=${me} idx=${i + 1} total=${report.questions.length} />`)}
...
function PulseQuestionBlock({ q, pid, me, idx, total }) {
  const blk = q.block || {};
  ...
  return html`
    <div class="q-block">
      ${/* a pulse IS a sequence, so the number carries something true — and it gives the
            eye a fixed left edge to find the start of each question against. */ ""}
      <div class="prq-head">
        ${idx ? html`<span class="prq-num">Q${idx}<span class="prq-of">/${total}</span></span>` : null}
        <h4 class="prq-title">${q.title}</h4>
      </div>
      ${blk.suppressed ? html`
            ...
            <div style=${{ marginTop: "var(--s2)" }}>
              ${!com ? html`<button class="btn small" onClick=${askAI}><${Icon} name="sparkle" size=${12} /> Commentary</button>` :
                ...
                <div class="def-box" style=${{ marginTop: "var(--s2)" }}>
                  ${/* "measures" restates the question verbatim, and on a pulse report the question is
                      the line directly above this box — so it is read twice and says nothing new.
                      Dropped here only; the metric page still shows it, where the definition
