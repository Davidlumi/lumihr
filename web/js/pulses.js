...
function PulseReport({ report, pid, me }) {
  ...
  const onCom = (qid, parts) => setComs(c => ({ ...c, [qid]: parts }));
    ...
    <div class="pulse-report-doc" style=${{ margin: "var(--s4) 0" }}>
      ...
      <div class="bench-grid pulse-grid-cards">
        ${report.questions.map((q, i) => html`<${PulseQuestionBlock} key=${q.question_id} q=${q}
           ...
           com=${coms[q.question_id]} onCom=${onCom} />`)}
      </div>
...
function PulseQuestionBlock({ q, pid, me, idx, total, report, com, onCom }) {
  const blk = q.block || {};
  const [expanded, setExpanded] = useState(false);
  // the read lives on the REPORT, not the card: "Print / save as PDF" writes every
  // question's observation before printing, and a card that owned its own copy would
  // have hidden that work from it
  const askAI = async () => {
    try { onCom(q.question_id, "…"); onCom(q.question_id, await api("/api/pulses/" + pid + "/commentary", { method: "POST", body: { question_id: q.question_id } })); }
    catch (e) { onCom(q.question_id, null); toast(e.message, "error"); }
  };
  ...
  const downloadOne = () => downloadPulseCsv({ ...report, questions: [q],
    name: (report.name || "pulse") + " — " + (q.title || "").slice(0, 40) });
  const share = () => {
  ...
  return html`
    <div class="card bench-card stacked pulse-card" id=${"pq-" + q.question_id}>
      ...
      <div class="bench-foot">
        ...
        <div class="card-tools no-print">
          ...
          ${me.features && me.features.pulse_ai && html`
            <button class=${"iconbtn" + (com ? " on" : "")} title="What this means for you"
              aria-label="Commentary" onClick=${() => (com ? onCom(q.question_id, null) : askAI())}>
              <${Icon} name="sparkle" size=${15} /></button>`}
          <button class="iconbtn" title="Download this question (CSV)" aria-label="Download question"
            onClick=${downloadOne}><${Icon} name="download" size=${15} /></button>
          <button class="iconbtn" title="Copy link to this pulse" aria-label="Copy link"
