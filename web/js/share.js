...
function ShareApp() {
  ...
  if (data.error) return html`
    <div class="auth-wrap"><div class="card auth-card" style=${{ textAlign: "center" }}>
      ...
      <div style=${{ margin: "var(--s4) 0 var(--s2)", color: "var(--ink-faint)" }}><${Icon} name="info" size=${28} /></div>
      <b>This share link is no longer available</b>
      <p class="caption">It may have expired or been revoked.</p>
      <p class="caption"><a href="/">What is lumi?</a> · Questions? hello@lumihr.co.uk</p>
    </div></div>`;
  ...
  return html`
    <div class="share-dashboard" style=${{ maxWidth: "1180px", margin: "0 auto", padding: "var(--s5) var(--s4)" }}>
      ...
      <div class="card banner" style=${{ marginBottom: "var(--s4)" }}>
        <div>
          <div class="section-title" style=${{ marginBottom: "var(--s1)" }}>Above the market median on ${h.above_median} of ${h.comparable_metrics} comparable metrics</div>
      ...
      <div class="bench-grid">
        ${/* §4.9(5): outside viewers get no member CTAs — readOnly hides "Add data" /
              "Answer this" and shows honest not-answered notes instead. */ ""}
        ${data.cards.map(c => html`<${Guarded} key=${c.id}><${BenchmarkCard} card=${c} prefs=${{}} readOnly=${true} /><//>`)}
      </div>
      <div class="caption" style=${{ margin: "var(--s5) 0", textAlign: "center" }}>
        Generated by <a href="/">lumi</a> · UK reward benchmarking · percentiles use linear interpolation · n &lt; 5 suppressed
        throughout · shared links can expire or be revoked
      </div>
    </div>`;
}
